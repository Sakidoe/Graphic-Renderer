...
void main() {
	...
	if (filter_mode == 0)
	{
		vec4 inputColor = texture(color_texture, o_texture_coord); //access the color component of your FBO
    ...
    {
		float depth = texture(depth_texture, o_texture_coord).r; //access the depth component of your FBO
	...
	else if (filter_mode == 2)
	{
		...
		vec4 inputColor = texture(color_texture, o_texture_coord); //access the color component of your FBO
		float g = sobel(color_texture, o_texture_coord); //calling the sobel function with correct params.

		//correct fragment depth values
		gl_FragDepth = texture(depth_texture, o_texture_coord).r; // setting var fragdepth to what we get in the depth texture

		// Final color after sobel filter
		o_fragColor = mix(inputColor, edgeColor, g);
		o_fragColor.a = inputColor.a; // Ensure that the alpha component remains unchanged
	}
