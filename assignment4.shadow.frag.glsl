...
float computeVisibility(vec4 shadow_coord, sampler2D shadow_tex)
{
    ...
    //first divide the xyz coords of shadow coord by the w
    vec3 shadow_coord_proj = shadow_coord.xyz / shadow_coord.w;
    //then find the texture visibility into a var
    float visibility = texture(shadow_tex, shadow_coord_proj.xy).r;
    //incorporate the bias into the z factor of the xyz/w
    float depth_from_light = shadow_coord_proj.z - bias;
    //if the depth resulting depth from the light is greater than the visibility, there is /isnt shadow.
    float shadow = depth_from_light > visibility  ? 0.1 : 1.0;
...
void main() {
    ...
    //o_fragColor = vec4(vec3(1.0,0.0,0.0), 1.0);
