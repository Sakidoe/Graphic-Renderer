...
void main() {
    ...
    o_shadow_coord_directional = bias_matrix * u_shadow_pv_directional * vec4(vertex_position_world.xyz, 1.0); //the calculation is just the bias matrix * pv * vertex position world
    o_shadow_coord_point = bias_matrix * u_shadow_pv_point * vec4(vertex_position_world.xyz, 1.0);
